#include "yolo.hpp"
InferenceEngine::InferenceEngine(const std::string &modelpath, const cv::Size &inputsize = {640,640}, const std::string &classfile = "../VisDroneClasses.txt", const bool useGPU = false){
    modelpath_ = modelpath;
    classfile_ = classfile;
    modelShape = inputsize;
    cudaEnabled = useGPU;
    loadModel(modelpath, useGPU);
    loadClassList(classfile);
}
std::vector<Detection> InferenceEngine::runInference(const cv::Mat &frame){
    ...
    for(int i  = 0; i<rows;i++){
        ...
        else // yolov5
        {
            ...
            if (confidence >= modelConfThreshold)
            {
...
void InferenceEngine::loadClassList(const std::string &filename){
...
void InferenceEngine::loadModel(const std::string &modelpath, const bool useGPU){
    if (useGPU) {
...
cv::Mat InferenceEngine::formatToSquare(const cv::Mat &source, int *pad_x, int *pad_y, float *scale)
{
    int col = source.cols;
    int row = source.rows;
